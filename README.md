...
├── config.json            # Centralized model hyperparameter configuration
...
To run the entire end-to-end machine learning pipeline without installing Python, creating virtual environments, or downloading individual libraries, the complete lifecycle has been packaged into a lightweight, self-contained **Docker image** hosted on Docker Hub.
...
Open the terminal (Command Prompt, PowerShell, or bash) and download the pre-packaged image:
```bash
